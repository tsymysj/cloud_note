...
public class Page<T> {
    ...
    private Integer pageNum;//当前页（前台未传递，默认第一页）
    private Integer pageSize;//每页的显示数量
    private long totalCount;//总记录数（后台数据库中查询到的；）
    ...
    private Integer startNavPage;//导航开始页（当前页-5；如果当前页-5小于1，则导航开始页为1，此时导航结束页为导航开始数+9，导航结束页为导航开始数+9大于总页数，则导航结束页为总页数）
    private Integer endNavPage;//导航结束页 （当前页+4，如果当前页+4大于总页数，则导航结束页为总页数，此时导航开始页为导航结束页-9，如果导航开始页为导航结束页-9小于1，则导航开始页为1）
    ...
    public Page(Integer pageNum, Integer pageSize, long totalCount) {
        ...
        if (this.endNavPage > totalPages){
            ...
            //，此时导航开始页为导航结束页-9，如果导航开始页为导航结束页-9小于1，则导航开始页为1
